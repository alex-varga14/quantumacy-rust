...
| `qkd-core` / `qkd-network` | **Simulated qubits, real classical protocol** | There is no quantum hardware: qubit preparation/transmission/measurement are simulated (as in upstream QKDSimkit), so derived keys are only as secret as the classical RNG. The classical side is now real, however: peers run as separate processes over TCP with an HMAC-SHA256-authenticated channel (QKD's hard requirement), true two-party CASCADE reconciliation with per-bit leakage accounting, and Toeplitz-hash privacy amplification that subtracts the measured leakage. Residual: channel sequence numbers are per-connection only — cross-session replay needs a session nonce (tracked). |
